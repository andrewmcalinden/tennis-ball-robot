...
//-----------------------
PCA9685_motorHat::PCA9685_motorHat(uint8_t addr) {
  _addr = addr;
  _bus=1;
  _pwm.init(_bus, _addr);
  usleep(1000 * 100);
}

//-----------------------
void PCA9685_motorHat::begin(uint16_t freq) {
  // init PWM w/_freq
...
//-----------------------
void PCA9685_motorHat::setPWM(uint8_t pin, uint16_t value) {
  if (value > 4095) {
...
//-----------------------
void PCA9685_motorHat::setPin(uint8_t pin, bool value) {
  if (value == LOW)
    _pwm.setPWM(pin, 0, 0);
  else
    _pwm.setPWM(pin, 4096, 0);
...
//-----------------------
Adafruit_DCMotor *PCA9685_motorHat::getMotor(uint8_t num) {
  // input 'num' is DC motor ID : 1-4
  // later in this code 'num' is 0-3 - that is a messy approach
...
//-----------------------
Adafruit_StepperMotor *PCA9685_motorHat::getStepper(uint16_t steps, uint8_t num) {
...
void Adafruit_DCMotor::run(uint8_t cmd) {
  switch (cmd) {
  case FORWARD:
    MC->setPin(IN2pin, PCA9685_motorHat::LOW);  // take low first to avoid 'break'
    MC->setPin(IN1pin, PCA9685_motorHat::HIGH);
    break;
  case BACKWARD:
    MC->setPin(IN1pin, PCA9685_motorHat::LOW);  // take low first to avoid 'break'
    MC->setPin(IN2pin, PCA9685_motorHat::HIGH);
    break;
  case RELEASE:
    MC->setPin(IN1pin, PCA9685_motorHat::LOW);
    MC->setPin(IN2pin, PCA9685_motorHat::LOW);
    break;
  }
}
...
void Adafruit_StepperMotor::release(void) {
  MC->setPin(AIN1pin, PCA9685_motorHat::LOW);
  MC->setPin(AIN2pin, PCA9685_motorHat::LOW);
  MC->setPin(BIN1pin, PCA9685_motorHat::LOW);
  MC->setPin(BIN2pin, PCA9685_motorHat::LOW);
  MC->setPWM(PWMApin, 0);
  MC->setPWM(PWMBpin, 0);
}
...
uint8_t Adafruit_StepperMotor::onestep(uint8_t dir, uint8_t style) {
  ...
  if (latch_state & 0x1) {
   // Serial.println(AIN2pin);
    MC->setPin(AIN2pin, PCA9685_motorHat::HIGH);
  } else {
    MC->setPin(AIN2pin, PCA9685_motorHat::LOW);
  }
  if (latch_state & 0x2) {
    MC->setPin(BIN1pin, PCA9685_motorHat::HIGH);
   // Serial.println(BIN1pin);
  } else {
    MC->setPin(BIN1pin, PCA9685_motorHat::LOW);
  }
  if (latch_state & 0x4) {
    MC->setPin(AIN1pin, PCA9685_motorHat::HIGH);
   // Serial.println(AIN1pin);
  } else {
    MC->setPin(AIN1pin, PCA9685_motorHat::LOW);
  }
  if (latch_state & 0x8) {
    MC->setPin(BIN2pin, PCA9685_motorHat::HIGH);
